...
module.exports = {
  ...
  entry: path.join(sourceDir, 'entry.js'),
  output: {
    filename: isProd ? 'bundle.[contenthash].js' : 'bundle.js', // Use [contenthash] for better cache management
    path: buildDir,
    publicPath: '/',
  },
  resolve: {
  ...
  module: {
    rules: [
      ...
      },
      {
        ...
        use: [
          ...
          { loader: 'css-loader', options: { importLoaders: 1 } },
          {
            ...
            options: {
              postcssOptions: {
                plugins: [postcssPresetEnv(), isProd ? cssnano() : null].filter(Boolean),
              },
            },
          },
          'sass-loader',
      ...
      },
      {
        ...
        type: 'asset/resource',
        generator: {
          filename: 'images/[name][ext]', // Output images in a separate folder
        },
      },
    ],
  },
  plugins: [
    new HtmlWebpackPlugin({
      template: path.join(sourceDir, 'views', 'layout', 'template.hbs'),
      filename: 'index.html', // Change the filename to output directly in the build directory
      inject: 'body', // Ensures scripts are injected at the end of the body
      minify: isProd, // Minifies HTML in production mode
    }),
    new HandlebarsPlugin({
      htmlWebpackPlugin: {
        enabled: true,
    ...
    ...(isProd ? prodPlugins : []), // Only add production-specific plugins when needed
  ],
};
